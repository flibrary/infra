...
in {
    ...
    mastodon = {
      ...
      s3 = {
        secretKeyPath = lib.mkOption {
          description = "Configure path to your S3 secret key";
          type = lib.types.nullOr lib.types.path;
          default = null;
        };
        accessKeyPath = lib.mkOption {
          description = "Configure path to your S3 access key";
          type = lib.types.nullOr lib.types.path;
          default = null;
        };
      };
      ...
      package = lib.mkOption {
        ...
        default = pkgs.mastodon.overrideAttrs (oldAttrs: rec {
          postPatch = ''
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.mastodon-init-dirs = {
      ...
      '' else
        "") + (if cfg.s3.secretKeyPath != null then ''
        AWS_SECRET_ACCESS_KEY="$(cat ${cfg.s3.secretKeyPath})"
      '' else
        "") + (if cfg.s3.accessKeyPath != null then ''
        AWS_ACCESS_KEY_ID="$(cat ${cfg.s3.accessKeyPath})"
      '' else
        "") + ''
          EOF
        '';
    ...
    systemd.services.mastodon-streaming = {
      ...
      serviceConfig = {
        ...
        SystemCallFilter = "~" + lib.concatStringsSep " "
          (systemCallsList ++ [ "@privileged" "@resources" ]);
      } // cfgService;
    };
